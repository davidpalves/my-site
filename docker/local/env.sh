...
build the base image from root dir of repo:
    docker build --no-cache -t service/mysite:1.0.0 -f ./docker/local/mysite/Dockerfile ./
...
get-ip() {
    for CONTAINER_HOST in $1;do
        ...
        ADD_PROTOCOL=
        check_mysite="mysite"
        if _contains "${check_mysite}" "${CONTAINER_HOST}"; then
          ADD_PROTOCOL="http://"
          ADD_PORT=":8000/ or try 127.0.0.1"
        fi
        check_db="postgres"
        if _contains "${check_db}" "${CONTAINER_HOST}"; then
          ADD_PROTOCOL="postgres://mysite:mysite@"
          ADD_PORT=":5432/"
        fi
